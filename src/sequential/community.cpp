...
void Community::display_community_graphs(int level) {
    vector<int> renumber(size, -1);
    for (int node = 0; node < size; node++) {
        renumber[n2c[node]]++;
    }
    ...
    c2n.resize(final);
    for (int i = 0; i < size; i++) {
        c2n[renumber[n2c[i]]].push_back(i);
    }

    for (int i = 0; i < final; i++) {

        std::ostringstream oss;
        oss << "Community_" << level << "_" << i << ".txt";

        ofstream foutput;
        foutput.open(oss.str().c_str(), fstream::out);
        ...
        for (int j = 0; j < c2n[i].size(); j++) {
            ...
            for (int j = 0; j < deg; j++) {
                int neigh = *(p.first + j);
                if (n2c[neigh] == n2c[node]) {

                    if (g.weights.size() != 0)
                        foutput << " (" << *(p.first + j) << " " << *(p.second + j) << ")";
                    else
                        foutput << " (" << *(p.first + j) <<" " << 1<<")";
